...
12. **Programmatic CI Scope Guarding:** Enforce prompt constraints at the CI level, not just in the prompt — a workflow step that diffs the PR against `restricted_files` in `.agent/jules.yml` and fails the build on any match. (Not yet implemented as a standalone workflow in this repo — add one before relying on the prompt-level rule alone.)
