...
pub fn tick(
    ...
    // 1. Kiểm tra Sensor Timeout (> 90 giây không nhận được packet mới)
    if now_ms.saturating_sub(sensor_last_update_ms) > 90_000 {
        if !matches!(ctx.phase, SystemPhase::Fault(_)) {
            ...
            result.events.push(OrchestratorEvent::SetMistValve { on: false });
    ...
    // 3. Push mẫu vào Stabilizer tracker trong các phase sục trộn / ổn định
    if matches!(
        ctx.phase,
    ...
    // 4. Delegate sang Phase Handler tương ứng
    ...
    // 5. Tick Peripheral Controllers (Osaka, Phun sương, Sục trộn)
    if !matches!(
        ctx.phase,
...
fn merge_tick_results(mut base: TickResult, addition: TickResult) -> TickResult {
    ...
    if addition.delta.phase.is_some() { base.delta.phase = addition.delta.phase; }
    if addition.delta.phase_start_ms.is_some() { base.delta.phase_start_ms = addition.delta.phase_start_ms; }
    if addition.delta.phase_finish_ms.is_some() { base.delta.phase_finish_ms = addition.delta.phase_finish_ms; }
    if addition.delta.peripherals.is_some() { base.delta.peripherals = addition.delta.peripherals; }
    if addition.delta.calibration.is_some() { base.delta.calibration = addition.delta.calibration; }
    if addition.delta.dosing_cycle_count_increment { base.delta.dosing_cycle_count_increment = true; }
    if addition.delta.reset_stabilizer { base.delta.reset_stabilizer = true; }
    if addition.delta.last_water_change_sec.is_some() { base.delta.last_water_change_sec = addition.delta.last_water_change_sec; }
    if addition.delta.next_water_change_trigger_sec.is_some() { base.delta.next_water_change_trigger_sec = addition.delta.next_water_change_trigger_sec; }
    if addition.delta.water_change_cron.is_some() { base.delta.water_change_cron = addition.delta.water_change_cron; }
    if addition.delta.reset_safety_budget { base.delta.reset_safety_budget = true; }
    if addition.delta.safety_override_until.is_some() { base.delta.safety_override_until = addition.delta.safety_override_until; }
    if addition.delta.manual_pump_timeout.is_some() { base.delta.manual_pump_timeout = addition.delta.manual_pump_timeout; }
    if addition.delta.manual_pump_timeout_clear.is_some() { base.delta.manual_pump_timeout_clear = addition.delta.manual_pump_timeout_clear; }
...
fn tick_peripheral_systems(
    ...
    } else if let Some(existing) = result.delta.peripherals.take() {
...
fn stop_automation_if_needed(mut result: TickResult, ctx: &SystemContext) -> TickResult {
    ...
    if automation_was_active {
        result.events.push(OrchestratorEvent::SetWaterPump { direction: WaterDirection::Stop });
        result.events.push(OrchestratorEvent::SetMistValve { on: false });
        result.events.push(OrchestratorEvent::SetOsakaPump { pwm_percent: 0 });
        result.events.push(OrchestratorEvent::SetDosingPump { pump: crate::core::fsm::events::DosingPumpTarget::NutrientA, on: false, pwm_percent: 0 });
        result.events.push(OrchestratorEvent::SetDosingPump { pump: crate::core::fsm::events::DosingPumpTarget::NutrientB, on: false, pwm_percent: 0 });
        result.events.push(OrchestratorEvent::SetDosingPump { pump: crate::core::fsm::events::DosingPumpTarget::PhUp, on: false, pwm_percent: 0 });
        result.events.push(OrchestratorEvent::SetDosingPump { pump: crate::core::fsm::events::DosingPumpTarget::PhDown, on: false, pwm_percent: 0 });
...
fn merge_peripheral_deltas(
    ...
    if addition.osaka_pump.is_some() { base.osaka_pump = addition.osaka_pump; }
    if addition.osaka_pwm.is_some() { base.osaka_pwm = addition.osaka_pwm; }
    if addition.is_misting_active.is_some() { base.is_misting_active = addition.is_misting_active; }
    if addition.last_mist_toggle_time.is_some() { base.last_mist_toggle_time = addition.last_mist_toggle_time; }
    if addition.mist_valve.is_some() { base.mist_valve = addition.mist_valve; }
    if addition.is_scheduled_mixing_active.is_some() { base.is_scheduled_mixing_active = addition.is_scheduled_mixing_active; }
    if addition.last_mixing_start_sec.is_some() { base.last_mixing_start_sec = addition.last_mixing_start_sec; }
    if addition.water_pump_in.is_some() { base.water_pump_in = addition.water_pump_in; }
    if addition.water_pump_out.is_some() { base.water_pump_out = addition.water_pump_out; }
    if addition.pump_a.is_some() { base.pump_a = addition.pump_a; }
    if addition.pump_b.is_some() { base.pump_b = addition.pump_b; }
    if addition.ph_up.is_some() { base.ph_up = addition.ph_up; }
    if addition.ph_down.is_some() { base.ph_down = addition.ph_down; }
    if addition.last_ec_before_dose.is_some() { base.last_ec_before_dose = addition.last_ec_before_dose; }
    if addition.last_ph_before_dose.is_some() { base.last_ph_before_dose = addition.last_ph_before_dose; }
    if addition.misting_started_by_dosing.is_some() { base.misting_started_by_dosing = addition.misting_started_by_dosing; }
    base
}
