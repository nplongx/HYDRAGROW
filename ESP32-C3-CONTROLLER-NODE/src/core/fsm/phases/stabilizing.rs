    ...
    fn tick(
        ...
        if let Some(sample) = ctx.calibration.finalize() {
            ...
            if did_learn {
                ...
                result.events.push(OrchestratorEvent::PublishCalibrationUpdate);
            } else {
                warn!(
...
fn build_dosing_cycle_event(
    ...
    let kalman = if did_learn {
        Some(KalmanLearningData {
            ...
            ph_down_gain_after: ctx.tuner.gain_learner.effective_ph_down_gain(config.ph_shift_down_per_ml),
