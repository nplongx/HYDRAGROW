...
fn user_id_from(req: &HttpRequest) -> Option<i64> {
    req.extensions()
        .get::<AuthContext>()
...
pub async fn claim_device(
    ...
    if device_id.is_empty() {
        return HttpResponse::BadRequest().json(serde_json::json!({"error": "device_id không được rỗng"}));
    }
    ...
    match device_ownership::claim_device(
        ...
        Err(e) => {
            ...
            HttpResponse::InternalServerError().json(serde_json::json!({"error": "Không thể claim thiết bị"}))
        }
    }
}
...
pub async fn unclaim_device(
    ...
    match device_ownership::unclaim_device(&app_state.pg_pool, user_id, &device_id).await {
        Ok(0) => HttpResponse::NotFound().json(serde_json::json!({"error": "Không tìm thấy liên kết"})),
        Ok(_) => HttpResponse::Ok().json(serde_json::json!({"status": "unclaimed", "device_id": device_id})),
        Err(e) => {
            tracing::error!(?e, "Lỗi unclaim device");
            HttpResponse::InternalServerError().json(serde_json::json!({"error": "Không thể unclaim"}))
        }
    }
}
...
pub async fn list_my_devices(
    ...
    match device_ownership::list_devices_for_user(&app_state.pg_pool, user_id).await {
        ...
        Err(e) => {
            ...
            HttpResponse::InternalServerError().json(serde_json::json!({"error": "Không thể lấy danh sách"}))
        }
    }
}
...
pub fn init_routes(cfg: &mut web::ServiceConfig) {
    cfg.route("/devices", web::get().to(list_my_devices))
       ...
       .route("/devices/{device_id}/claim", web::delete().to(unclaim_device));
}
