...
pub async fn replace_wifi_metadata(
    ...
    for entry in entries {
        sqlx::query(
            "INSERT INTO device_wifi_config (device_id, ssid, priority, config_version) VALUES ($1, $2, $3, $4)",
        )
        .bind(device_id)
        .bind(&entry.ssid)
        .bind(entry.priority)
...
pub async fn get_wifi_metadata(
    ...
    let rows: Vec<WifiSsidRow> = sqlx::query_as(
        "SELECT ssid, priority, config_version FROM device_wifi_config WHERE device_id = $1 ORDER BY priority ASC",
    )
    .bind(device_id)
    .fetch_all(pool)
    .await
...
/// Record the desired delivery state for a device config version.
pub async fn set_delivery_state(
    ...
    sqlx::query(
        "INSERT INTO device_wifi_delivery (device_id, config_version, desired_state, last_result, updated_at, applied_at)
         VALUES ($1, $2, $3, $4, CURRENT_TIMESTAMP, CASE WHEN $3 = 'applied' THEN CURRENT_TIMESTAMP ELSE NULL END)
         ON CONFLICT (device_id) DO UPDATE SET
           config_version = EXCLUDED.config_version,
           desired_state = EXCLUDED.desired_state,
           last_result = EXCLUDED.last_result,
           updated_at = CURRENT_TIMESTAMP,
           applied_at = CASE WHEN EXCLUDED.desired_state = 'applied' THEN CURRENT_TIMESTAMP ELSE device_wifi_delivery.applied_at END",
    )
    .bind(device_id)
    .bind(version)
    .bind(state)
...
/// Load the tracked delivery row, if any.
pub async fn get_delivery(
    pool: &PgPool,
    device_id: &str,
) -> DbResult<Option<(i64, String, Option<String>)>> {
    let row: Option<DeliveryRow> = sqlx::query_as(
        "SELECT config_version, desired_state, last_result FROM device_wifi_delivery WHERE device_id = $1",
    )
    .bind(device_id)
    .fetch_optional(pool)
    .await
