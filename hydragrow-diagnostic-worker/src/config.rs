...
mod tests {
    ...
    fn from_pairs_reads_required_and_applies_defaults() {
        ...
        assert_eq!(config.max_tool_round_trips, 5);
        assert_eq!(config.max_input_tokens, 8000);
        assert_eq!(config.max_output_tokens, 1024);
        assert_eq!(config.per_call_timeout_secs, 25);
        assert_eq!(config.diagnosis_wall_clock_budget_secs, 60);
        assert_eq!(config.llm_provider, "openrouter");
        assert_eq!(config.llm_model, "nvidia/nemotron-3-super-120b-a12b:free");
    }

    #[test]
    fn from_pairs_errors_without_openrouter_api_key() {
        let vars = [
            ("DIAGNOSTIC_BACKEND_URL", "https://api.example.com"),
            ("DIAGNOSTIC_API_KEY", "svc_test"),
        ];
