    ...
    pub fn from_pairs(pairs: &[(&str, &str)]) -> anyhow::Result<Self> {
        ...
        if provider == "openrouter" && openrouter_api_key.is_empty() {
            return Err(anyhow::anyhow!(
                "OPENROUTER_API_KEY is required when DIAGNOSTIC_LLM_PROVIDER=openrouter"
            ));
        }
        Ok(Self {
            backend_url,
            ...
            per_call_timeout_secs: get_num("DIAGNOSTIC_PER_CALL_TIMEOUT_SECS", 60)?,
            diagnosis_wall_clock_budget_secs: get_num("DIAGNOSTIC_WALL_CLOCK_BUDGET_SECS", 180)?,
...
mod tests {
    ...
    fn local_defaults_without_openrouter_key() {
        ...
        assert_eq!(config.diagnosis_wall_clock_budget_secs, 180);
    }
    #[test]
    fn custom_timeouts_parsed() {
        let vars = [
            ("DIAGNOSTIC_BACKEND_URL", "https://api.example.com"),
            ("DIAGNOSTIC_API_KEY", "svc_test"),
            ("DIAGNOSTIC_PER_CALL_TIMEOUT_SECS", "45"),
            ("DIAGNOSTIC_WALL_CLOCK_BUDGET_SECS", "90"),
        ];
        let config = WorkerConfig::from_pairs(&vars).unwrap();
        assert_eq!(config.per_call_timeout_secs, 45);
        assert_eq!(config.diagnosis_wall_clock_budget_secs, 90);
    }
    #[test]
    fn openrouter_requires_key() {
        let vars = [
            ("DIAGNOSTIC_BACKEND_URL", "https://api.example.com"),
            ("DIAGNOSTIC_API_KEY", "svc_test"),
            ("DIAGNOSTIC_LLM_PROVIDER", "openrouter"),
        ];
