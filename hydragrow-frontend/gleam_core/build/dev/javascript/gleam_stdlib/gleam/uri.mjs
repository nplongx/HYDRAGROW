...
function query_pair(pair) {
  return (percent_encode_query(pair[0]) + "=") + percent_encode_query(pair[1]);
}
...
export function to_string(uri) {
  ...
  if ($1 instanceof Some) {
    ...
    if ($2 instanceof Some) {
      let userinfo = $2[0];
      _block$2 = (out$1 + userinfo) + "@";
    } else {
      _block$2 = out$1;
    }
    let out$2 = _block$2;
    let out$3 = out$2 + host;
    let _block$3;
    let $3 = uri.port;
    if ($3 instanceof Some) {
      let port = $3[0];
      _block$3 = (out$3 + ":") + $int.to_string(port);
    } else {
      _block$3 = out$3;
    }
    let out$4 = _block$3;
...
export function origin(uri) {
  ...
  if (host instanceof Some && scheme instanceof Some) {
    ...
    if ($ === "https" && isEqual(port, new Some(443))) {
      let h = host[0];
      return new Ok("https://" + h);
    } else if ($ === "http" && isEqual(port, new Some(80))) {
      let h = host[0];
      return new Ok("http://" + h);
    } else {
      let s = $;
      if ((s === "http") || (s === "https")) {
        let h = host[0];
        if (port instanceof Some) {
          let p = port[0];
          return new Ok((((s + "://") + h) + ":") + $int.to_string(p));
        } else {
          return new Ok((s + "://") + h);
        }
      } else {
