...
export function extractActions(workspace: Blockly.Workspace): Action[] {
  const alerts: Action[] = workspace
    .getBlocksByType("hydragrow_alert_action", false)
    .map((block) => ({
      type: "alert" as const,
      level: block.getFieldValue("LEVEL") as "info" | "warning" | "error",
      message: block.getFieldValue("MESSAGE"),
      // Blockly FieldCheckbox trả về chuỗi 'TRUE'/'FALSE', không phải boolean JS.
      notifyFcm: block.getFieldValue("NOTIFY_FCM") === "TRUE",
    }));
  const advances: Action[] = workspace
    .getBlocksByType("hydragrow_advance_stage_action", false)
    .map((block) => ({
      type: "advance_stage" as const,
      targetStageOffset: Number(block.getFieldValue("OFFSET")),
      reason: block.getFieldValue("REASON"),
    }));
  const doses: Action[] = workspace
    .getBlocksByType("hydragrow_dose_action", false)
    .map((block) => ({
      type: "dose" as const,
      pump: block.getFieldValue("PUMP") as
        | "PUMP_A"
        | "PUMP_B"
        | "PH_UP"
        | "PH_DOWN",
  ...
  const waters: Action[] = workspace
    .getBlocksByType("hydragrow_water_action", false)
    .map((block) => {
      const pump = block.getFieldValue("PUMP") as
        | "WATER_PUMP_IN"
        | "WATER_PUMP_OUT"
        | "MIST_VALVE"
        | "OSAKA_PUMP";
      if (block.getFieldValue("STATE") === "on") {
        return {
          type: "water_on" as const,
          pump,
          durationSec: Number(block.getFieldValue("DURATION_SEC")),
        };
      }
