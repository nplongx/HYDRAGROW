...
function LeafEditor({
  ...
  return (
    <div className="mb-2 flex gap-1">
      ...
      />
      <button type="button" className="text-xs text-red-600 cursor-pointer" onClick={onRemove}>
        ✕
      </button>
    </div>
  );
...
export function ConditionGroupEditor({ group, fields, onChange, isRoot }: ConditionGroupEditorProps): JSX.Element {
  ...
  return (
    <div className={isRoot ? '' : 'ml-3 border-l border-emerald-100 pl-3'}>
      <div className="mb-2 flex items-center gap-2">
        {!isRoot && <span className="text-xs text-emerald-800/60">Nhóm con</span>}
        <div className="flex overflow-hidden rounded border border-emerald-200 text-xs">
          <button
            type="button"
            aria-pressed={group.op === 'and'}
            className={`px-2 py-1 font-semibold ${group.op === 'and' ? 'bg-emerald-600 text-white' : 'bg-white text-emerald-800'}`}
          ...
          <button
            ...
            className={`px-2 py-1 font-semibold ${group.op === 'or' ? 'bg-emerald-600 text-white' : 'bg-white text-emerald-800'}`}
      ...
      {group.children.map((child, i) =>
        isGroup(child) ? (
          <ConditionGroupEditor key={i} group={child} fields={fields} onChange={(next) => setChild(i, next)} />
        ) : (
          <LeafEditor
            key={i}
            condition={child}
      ...
      <div className="flex gap-3 text-xs">
        <button type="button" className="text-emerald-700 cursor-pointer" onClick={addLeaf}>
          + Thêm điều kiện
        </button>
        <button type="button" className="text-emerald-700 cursor-pointer" onClick={addGroup}>
