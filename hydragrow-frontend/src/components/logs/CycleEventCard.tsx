    ...
    <div className="border border-line rounded-2xl p-4 bg-gradient-to-r from-cyan-500/5 to-transparent shadow-sm">
      <div className="flex items-start justify-between gap-3 mb-3">
        <div>
          ...
          <div className="flex items-center gap-1.5 mt-1 text-[10px] font-mono text-text-muted">
            <Link2 size={10} />
            <span>{cycleId}</span>
            <span>· {sorted.length} bước</span>
          </div>
        </div>
        <time className="text-[10px] text-text-muted font-mono text-right shrink-0">
        ...
        {sorted.map((ev) => (
          <li key={ev.id} className="relative pl-3">
            ...
            <button
              ...
              className="text-left w-full text-xs text-primary-deep hover:text-primary font-medium"
            >
              {ev.title}
              {ev.message && ev.message !== ev.title && (
                <span className="block text-[11px] text-text-muted font-normal">{ev.message}</span>
              )}
            </button>
          </li>
        ))}
