...
export const EmergencyStopConfirmDialog = ({
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-emerald-950/40 backdrop-blur-sm flex items-end sm:items-center justify-center p-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm p-6 space-y-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="text-base font-bold text-emerald-950">Dừng khẩn cấp toàn hệ thống</h3>
        ...
        <div className="space-y-1.5">
          <p className="text-[10px] font-bold uppercase tracking-wider text-emerald-700/70">Thiết bị đang chạy sẽ bị tắt</p>
          {runningKeys.length === 0 ? (
            <p className="text-xs text-emerald-800/60 italic">Không có thiết bị nào đang chạy.</p>
          ) : (
        ...
        <div className="flex gap-2 pt-2">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-xl border border-emerald-100 text-emerald-800 text-sm font-bold hover:bg-emerald-50 transition-all disabled:opacity-50 cursor-pointer"
          >
            <X size={14} /> Huỷ
          </button>
          <button
