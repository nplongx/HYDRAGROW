...
export const CreateSeasonForm: React.FC<CreateSeasonFormProps> = ({ isLoading, onCreateSeason }) => {
  ...
  return (
    <div className="bg-white border border-line rounded-xl overflow-hidden mb-6 shadow-sm">
      <form onSubmit={handleSubmit} className="p-5 md:p-6 flex flex-col gap-5">
        <h2 className="text-base font-semibold text-primary-deep flex items-center gap-2 border-b border-line pb-4">
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-semibold text-primary-deep flex items-center gap-2">
              <Bookmark size={15} className="text-primary" />
              Công thức nuôi trồng (Recipe Template) <span className="text-error">*</span>
            </label>
            <select
              value={selectedRecipeId}
              onChange={(e) => handleSelectRecipe(e.target.value)}
              className="w-full bg-white border border-line text-primary-deep text-sm rounded-lg px-3 py-2.5 outline-none focus:border-primary cursor-pointer"
            >
          ...
          {selectedRecipe && (
            <div className="rounded-xl border border-line bg-surface-muted p-3.5 space-y-2 text-xs text-text-muted">
              <div className="flex items-center justify-between font-bold">
                <span>Giống cây: <b className="text-primary-deep uppercase">{selectedRecipe.crop}</b></span>
                <span>Số stage: {selectedRecipe.stages.length}</span>
              </div>
              <div className="flex flex-wrap gap-2 pt-1">
                {selectedRecipe.stages.map((st, i) => (
                  <span key={i} className="inline-flex items-center gap-1 bg-white border border-line px-2 py-1 rounded-md">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-sm font-medium text-primary-deep">Ghi chú ban đầu</label>
            <textarea
              rows={2}
        ...
        <button
          ...
          className="w-full py-3 bg-primary hover:bg-primary-deep text-white rounded-lg font-bold text-sm transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
        >
          <CheckCircle size={16} />
