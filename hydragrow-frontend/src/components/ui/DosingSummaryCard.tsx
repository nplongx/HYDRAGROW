...
export const DosingSummaryCard: React.FC<DosingSummaryCardProps> = ({ totalCount, lastDosedAt }) => {
  ...
  return (
    <div className="ui-card space-y-1.5">
      <h4 className="text-sm font-bold text-primary-deep">Châm dinh dưỡng hôm nay</h4>
      {lastDosedLabel ? (
        <p className="text-[13px] text-text-muted">{totalCount} lần · lần cuối {lastDosedLabel}</p>
      ) : (
        <p className="text-[13px] text-text-muted">Chưa ghi nhận lần châm nào hôm nay</p>
      )}
    </div>
  );
};
