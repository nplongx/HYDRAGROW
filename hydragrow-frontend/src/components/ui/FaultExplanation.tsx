      ...
      <div className="bg-white border border-red-200 rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-4 animate-in zoom-in-95 duration-200">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-primary-deep">Chi tiết sự cố</h3>
              <p className="text-xs font-mono font-semibold text-red-600 mt-0.5">Mã lỗi: {code}</p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg text-text-muted hover:text-primary-deep hover:bg-soft transition-colors"
          >
            <X size={18} />
          </button>
        </div>
        ...
        {guide ? (
          <div className="space-y-3 pt-2">
            ...
            {guide.cause && (
              <div className="space-y-1">
                <span className="text-[11px] font-bold uppercase tracking-wider text-text-muted flex items-center gap-1.5">
                  <Info size={13} className="text-primary" /> Nguyên nhân khả dĩ
                </span>
                <p className="text-xs text-primary-deep bg-surface-muted border border-line p-2.5 rounded-lg leading-relaxed">
                  {guide.cause}
                </p>
              </div>
            )}
            {guide.action && (
              <div className="space-y-1">
                <span className="text-[11px] font-bold uppercase tracking-wider text-text-muted flex items-center gap-1.5">
                  <Wrench size={13} className="text-primary" /> Hướng dẫn khắc phục
                </span>
                <p className="text-xs font-semibold text-primary-deep bg-pill border border-line p-2.5 rounded-lg leading-relaxed">
                  {guide.action}
                </p>
              </div>
            )}
        ...
        <div className="pt-2 flex justify-end">
          <button
            onClick={onClose}
            className="px-4 py-2 bg-primary hover:bg-primary-deep text-white text-xs font-bold rounded-xl transition-colors"
          >
            Đóng
          </button>
        </div>
