...
export const FsmStatusBadge: React.FC<{ state?: string }> = ({ state }) => {
  ...
  if (rawState.startsWith('EmergencyStop:')) {
    ...
    return renderBadge(
      'bg-red-600 border border-red-500 animate-pulse',
      'text-white',
      `🛑 DỪNG KHẨN CẤP: ${reason}`,
    );
  }
  ...
  switch (rawState) {
    ...
    case 'EmergencyStop':
      return renderBadge('bg-red-600 border border-red-500', 'text-white', '🛑 DỪNG KHẨN CẤP'); // Fallback cho code cũ
    case 'WaterRefilling':
      return renderBadge(
    ...
    default:
      return renderBadge('bg-slate-700 border border-slate-600', 'text-slate-300', rawState);
  }
};
