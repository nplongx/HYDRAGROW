...
const AdvancedSpecsGrid = ({ dosing }: { dosing: any }) => {
  ...
  if (ecBefore != null) rows.push({ label: 'EC trước bão hòa', value: ecBefore.toFixed(2), accent: 'text-cyan-400' });
  if (ecAfter != null) rows.push({ label: 'EC sau bão hòa', value: ecAfter.toFixed(2), accent: 'text-cyan-400 font-bold' });
  ...
  if (dosing.target_ec != null) rows.push({ label: 'Ngưỡng EC mục tiêu', value: Number(dosing.target_ec).toFixed(2), accent: 'text-slate-300' });
  if (dosing.target_ph != null) rows.push({ label: 'Ngưỡng pH mục tiêu', value: Number(dosing.target_ph).toFixed(2), accent: 'text-slate-300' });
  ...
  return (
    <div className="mt-3 bg-slate-950/60 border border-slate-800/60 rounded-xl p-3 animate-in slide-in-from-top-2 duration-300">
      <div className="text-[9px] font-black text-slate-500 uppercase tracking-wider mb-2 flex items-center gap-1.5">
  ...
  return (
    <div className="flex items-start space-x-4 animate-in slide-in-from-bottom-4 duration-500" style={{ animationDelay: `${Math.min(index * 40, 400)}ms`, animationFillMode: 'both' }}>
      ...
      <div className="shrink-0 mt-3.5 relative z-10">
        <div className={`w-8 h-8 rounded-full border-4 border-slate-950 flex items-center justify-center shadow-lg 
      ...
      {/* Thẻ nội dung */}
      <div className="flex-1 bg-gradient-to-br from-slate-900/80 to-slate-900/40 backdrop-blur-md border border-slate-800/80 rounded-2xl p-4 hover:border-slate-700 transition-colors shadow-sm">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-2">
          <div className="space-y-1">
            ...
            <div className="flex flex-wrap items-center gap-2 pt-1 text-xs font-semibold">
              ...
              {record.ph_up_ml > 0 && <span className="text-purple-400 bg-purple-500/10 px-2 py-0.5 rounded border border-purple-500/20">pH↑: {record.ph_up_ml.toFixed(1)}ml</span>}
              {record.ph_down_ml > 0 && <span className="text-rose-400 bg-rose-500/10 px-2 py-0.5 rounded border border-rose-500/20">pH↓: {record.ph_down_ml.toFixed(1)}ml</span>}
              {(dosing.water_in_sec ?? 0) > 0 && <span className="text-blue-400 bg-blue-500/10 px-2 py-0.5 rounded border border-blue-500/20 flex items-center gap-1"><Waves size={10} /> Cấp {dosing.water_in_sec?.toFixed(1)}s</span>}
            </div>
          </div>
          <time className="text-[10px] text-slate-500 font-mono text-right whitespace-nowrap shrink-0">
            {date.toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })}
            <span className="block font-medium text-slate-600 mt-0.5">{date.toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit' })}</span>
          </time>
        </div>
...
const DosingHistory = () => {
  ...
  return (
    <div className="p-4 md:p-8 space-y-6 pb-28 max-w-4xl mx-auto text-slate-200">
      ...
      <div className="bg-slate-900/60 border border-slate-800/80 rounded-3xl p-4 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 backdrop-blur-md relative z-20">
        <div className="relative flex-1 max-w-xs">
          ...
          <div className="relative">
            <select
              ...
              className="w-full bg-slate-950 border border-slate-800 text-slate-200 text-sm font-semibold rounded-2xl pl-4 pr-10 py-2.5 appearance-none outline-none focus:border-indigo-500 disabled:opacity-50 cursor-pointer shadow-inner"
            >
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 disabled:opacity-40 text-white px-5 py-2.5 rounded-2xl border border-slate-700 transition-all font-bold text-xs uppercase tracking-wider shrink-0 mt-auto shadow-sm active:scale-95"
        >
          <Download size={14} className={history.length > 0 ? "text-emerald-400" : "text-slate-500"} />
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Niên vụ canh tác</p>
