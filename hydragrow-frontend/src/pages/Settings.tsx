  ...
  return (
    <div className="app-page max-w-5xl pb-36">
      ...
      <PageHeader
        ...
        subtitle="Điều chỉnh thông số vận hành cho trạm của bạn."
        icon={Settings2}
      />

      {/* Advanced Mode Banner */}
      <div className="ui-alert-warning rounded-2xl mb-6">
        <Settings2 size={18} className="shrink-0 mt-0.5" />
        <div className="flex-1 space-y-1">
          <div className="flex items-center justify-between">
            <h4 className="text-sm font-bold">Chế độ kỹ thuật</h4>
            <Switch checked={isAdvancedMode} onChange={setIsAdvancedMode} />
          </div>
          <p className="text-xs opacity-70">Bật để chỉnh thông số nâng cao: PWM, hệ số hiệu chỉnh, giới hạn an toàn.</p>
        </div>
      </div>

      <div className="space-y-6">
        <div className="ui-card space-y-4">
          <p className="farm-section-title">
            <LockKeyhole size={16} /> Tài khoản đăng nhập
          </p>
          <SubCard title="Tài khoản">
            <p>Đang đăng nhập: <strong>{user?.email ?? 'Không xác định'}</strong></p>
            <button type="button" onClick={() => logout()} className="ui-btn-secondary text-xs mt-2">
              Đăng xuất
            </button>
          </SubCard>
        </div>
        ...
        <div className="ui-card space-y-4">
          <p className="farm-section-title">
            <Network size={16} /> Thiết bị & Kết nối
          </p>
          <AccordionSection id="network" title="Cấu hình kết nối" icon={Network} isOpen={openSection === 'network'} onToggle={() => handleToggleSection('network')}>
        ...
        <div className="ui-card space-y-4">
          ...
          <AccordionSection id="wifi" title="Mạng WiFi thiết bị (ưu tiên)" icon={Network} isOpen={openSection === 'wifi'} onToggle={() => handleToggleSection('wifi')}>
            <div className="space-y-3 p-1">
              ...
              <button type="button" disabled={isSavingWifi} onClick={handleSaveWifiList} className="ui-btn-primary w-full">{isSavingWifi ? 'Đang gửi...' : 'Lưu danh sách WiFi (áp dụng sau khi khởi động lại)'}</button>
            </div>
          </AccordionSection>
        </div>
        ...
        <div className="ui-card space-y-4">
          <p className="farm-section-title">
            <Power size={16} /> Tổng quan vận hành
          </p>
          <AccordionSection id="general" title="Chế độ & Trạng thái" icon={Power} isOpen={openSection === 'general'} onToggle={() => handleToggleSection('general')}>
        ...
        <div className="ui-card space-y-4">
          <p className="farm-section-title">
            <Target size={16} /> Ngưỡng mục tiêu sinh trưởng
          </p>
          <AccordionSection id="growth" title="Cấu hình mục tiêu" icon={Target} isOpen={openSection === 'growth'} onToggle={() => handleToggleSection('growth')}>
        ...
        <div className="ui-card space-y-4">
          <p className="farm-section-title">
            <Waves size={16} /> Quản lý mực nước & xả nước
          </p>
          <AccordionSection id="water" title="Mực nước & Bơm xả" icon={Waves} isOpen={openSection === 'water'} onToggle={() => handleToggleSection('water')}>
        ...
        <div className="ui-card space-y-4">
          <p className="farm-section-title">
            <FlaskConical size={16} /> Hệ thống châm vi chất & định lượng
          </p>
          <AccordionSection id="dosing" title="Thông số bơm châm & khuấy" icon={FlaskConical} isOpen={openSection === 'dosing'} onToggle={() => handleToggleSection('dosing')}>
            {isAdvancedMode && (
        ...
        {isAdvancedMode && (
          <div className="ui-card space-y-4">
            <p className="farm-section-title">
              <ShieldAlert size={16} /> Giới hạn an toàn & Bảo vệ
            </p>
            <AccordionSection id="safety" title="Ngưỡng cảnh báo an toàn" icon={ShieldAlert} isOpen={openSection === 'safety'} onToggle={() => handleToggleSection('safety')}>
        ...
        <div className="ui-card space-y-4">
          <p className="farm-section-title">
            <Activity size={16} /> Hiệu chuẩn cảm biến
          </p>
          <AccordionSection id="sensor" title="Cảm biến & Hiệu chuẩn pH" icon={Activity} isOpen={openSection === 'sensor'} onToggle={() => handleToggleSection('sensor')}>
        ...
        <section className="mt-8 border-t pt-6 ui-card space-y-4">
          <p className="farm-section-title text-red-600">
            <ShieldAlert size={16} /> Vùng Nguy Hiểm
          </p>
      ...
      <div className="fixed bottom-[84px] md:bottom-[90px] left-0 right-0 z-40 pointer-events-none p-4 md:p-0 flex justify-center md:justify-end md:right-8">
        <button
          ...
          className="ui-btn-primary w-full md:w-auto pointer-events-auto px-8 py-3.5 shadow-[0_10px_30px_-10px_rgba(21,128,61,0.8)]"
        >
          {isSaving ? (
