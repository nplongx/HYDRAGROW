...
const backendLikeError = (field: string, detail: string) =>
  `Giá trị không hợp lệ cho ${field}: ${detail}.`;
...
const validateDosingConfig = (inputConfig: any): DosingValidationErrors => {
  ...
  if (!Number.isFinite(dosingPwm) || dosingPwm < 1 || dosingPwm > 100) {
    errors.dosing_pwm_percent = backendLikeError(
      'dosing_pwm_percent',
  ...
  if (scheduledDosingEnabled && (!Number.isFinite(doseA) || doseA < 0)) {
    errors.scheduled_dose_a_ml = backendLikeError(
      'scheduled_dose_a_ml',
      'phải lớn hơn hoặc bằng 0',
    );
  }
  if (scheduledDosingEnabled && (!Number.isFinite(doseB) || doseB < 0)) {
    errors.scheduled_dose_b_ml = backendLikeError(
      'scheduled_dose_b_ml',
      'phải lớn hơn hoặc bằng 0',
    );
  }
...
const Settings = () => {
  ...
  const [config, setConfig] = useState<any>({
    ...
    water_level_target: 80.0,
    water_level_max: 90.0,
    water_level_drain: 5.0,
    circulation_mode: 'always_on',
    ...
    dilute_drain_amount_cm: 5.0,
    scheduled_water_change_enabled: false,
    water_change_cron: '0 0 7 * * SUN',
    scheduled_drain_amount_cm: 10.0,

    tank_volume_l: 50.0,
    ec_gain_per_ml: 0.1,
    ph_shift_up_per_ml: 0.2,
    ph_shift_down_per_ml: 0.2,
    ec_step_ratio: 0.4,
    ph_step_ratio: 0.1,
  ...
  const [capturedPoints, setCapturedPoints] = useState<
    Record<number, { voltage: number; confidence: number; capturedAt: string }>
  >({});
  ...
  const calibrationSummary = (() => {
    ...
    const avgConfidence = confidenceList.length
      ...
      : 0;
    const spread =
      Number.isFinite(p7) && Number.isFinite(p4) ? Math.abs((p7 as number) - (p4 as number)) : 0;
  ...
  const hasSafetyWarningIn24h = systemEvents.some((ev: any) => {
    ...
    return (
      ...
      (category.includes('safe') || title.includes('cảnh báo') || title.includes('safety'))
    );
  });
  ...
  const calibrationDeviation = {
    ph_v7:
      calibrationSummary.ph_v7 !== null
        ? Number((calibrationSummary.ph_v7 - Number(config.ph_v7 || 0)).toFixed(3))
        : null,
    ph_v4:
      calibrationSummary.ph_v4 !== null
        ? Number((calibrationSummary.ph_v4 - Number(config.ph_v4 || 0)).toFixed(3))
        : null,
  };
    ...
    const loadConfig = async () => {
      try {
        ...
        try {
          ...
          console.error('Chưa load được store');
        }
        ...
        const unifiedData = await callApi(
          `/api/devices/${currentDeviceId}/config/unified`,
          'GET',
          null,
          settings,
        ).catch(() => null);
  ...
  const handleSave = async (configOverride?: any) => {
    ...
    try {
      ...
      if (Object.keys(saveValidationErrors).length > 0) {
        toast.error(
          'Không thể lưu: cấu hình dosing không hợp lệ. Vui lòng kiểm tra các trường đang báo lỗi.',
        );
        return;
      }
      ...
      const doseConf = {
        ...
        pump_ph_up_capacity_ml_per_sec: toNumberOr(
          savingConfig.pump_ph_up_capacity_ml_per_sec,
          1.2,
        ),
        pump_ph_down_capacity_ml_per_sec: toNumberOr(
          savingConfig.pump_ph_down_capacity_ml_per_sec,
          1.2,
        ),
        ...
        last_dynamic_update:
          typeof savingConfig.last_dynamic_update === 'string' && savingConfig.last_dynamic_update
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-950 relative overflow-hidden">
        ...
        <div className="flex flex-col items-center space-y-4 relative z-10">
          ...
          <span className="text-emerald-500/70 font-black tracking-widest text-xs uppercase animate-pulse">
            Đang tải cấu hình thiết bị...
          </span>
        </div>
      </div>
    );
    ...
    <div className="p-4 md:p-6 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-40 max-w-4xl mx-auto relative min-h-screen">
      ...
      <div className="space-y-4 relative z-10">
        ...
        <AccordionSection
          ...
          onToggle={() => handleToggleSection('network')}
        >
          <div className="space-y-3 bg-slate-900/30 p-4 rounded-2xl border border-white/5 shadow-inner">
            <InputGroup
              label="Mã Thiết Bị (Device ID)"
              type="text"
              value={appSettings.device_id}
              onChange={(e: InputEvent) =>
                setAppSettings({ ...appSettings, device_id: e.target.value })
              }
              desc="ID định danh cấp cho tủ điện"
            />
            <InputGroup
              label="Địa chỉ Máy Chủ (Backend URL)"
              type="text"
              value={appSettings.backend_url}
              onChange={(e: InputEvent) =>
                setAppSettings({ ...appSettings, backend_url: e.target.value })
              }
              desc="Ví dụ: http://192.168.1.5:8000"
            />
            <InputGroup
              label="Khóa Bảo Mật (API Key)"
              type="password"
              value={appSettings.api_key}
              onChange={(e: InputEvent) =>
                setAppSettings({ ...appSettings, api_key: e.target.value })
              }
            />
          </div>
        </AccordionSection>
        ...
        >
          <div className="space-y-4">
            <div
              className={`flex items-center justify-between p-4 rounded-2xl border transition-all duration-500 ${config.is_enabled ? 'bg-emerald-500/10 border-emerald-500/30 shadow-[0_0_20px_rgba(16,185,129,0.15)]' : 'bg-slate-900/50 border-slate-800'}`}
            >
              <div>
                <p
                  className={`text-sm font-black tracking-wide ${config.is_enabled ? 'text-emerald-400' : 'text-slate-400'}`}
                >
                  KÍCH HOẠT HỆ THỐNG
                </p>
                <p className="text-[10px] text-slate-500 font-bold uppercase mt-1">
                  Cho phép tủ điện chạy tự động
                </p>
              </div>
              <Switch
                isOn={config.is_enabled}
                onClick={(val) => setConfig({ ...config, is_enabled: val })}
                colorClass="bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]"
              />
            </div>

            <div
              className={`flex items-center justify-between p-4 rounded-2xl border transition-all duration-500 ${config.emergency_shutdown ? 'bg-rose-500/20 border-rose-500/50 shadow-[0_0_30px_rgba(244,63,94,0.3)] animate-pulse' : 'bg-slate-900/50 border-slate-800'}`}
            >
              <div className="flex items-start gap-3">
                <ShieldAlert
                  className={config.emergency_shutdown ? 'text-rose-400' : 'text-slate-600'}
                  size={20}
                />
                <div>
                  <p
                    className={`text-sm font-black tracking-wide ${config.emergency_shutdown ? 'text-rose-400' : 'text-slate-400'}`}
                  >
                    DỪNG KHẨN CẤP (E-STOP)
                  </p>
                  <p className="text-[10px] text-slate-500 font-bold uppercase mt-1">
                    Ngắt ngay lập tức mọi thiết bị điện
                  </p>
                </div>
              </div>
              <Switch
                isOn={config.emergency_shutdown}
                onClick={(val) => setConfig({ ...config, emergency_shutdown: val })}
            ...
            <div className="p-4 bg-slate-900/40 rounded-2xl border border-white/5 backdrop-blur-md">
              ...
              <div className="flex space-x-2 bg-slate-950/50 p-1.5 rounded-xl border border-slate-800/50 shadow-inner">
                <button
                  ...
                  className={`flex-1 py-3 rounded-lg text-xs font-black tracking-widest transition-all duration-300 ${
        ...
        >
          ...
          <SubCard
            title="Nhiệt Độ Trồng & Phun Sương Không Khí"
            className="mt-4 border-blue-500/20 bg-gradient-to-br from-blue-500/5 to-transparent"
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <InputGroup
                label="Nhiệt độ phòng tối ưu (°C)"
                step="0.5"
                value={config.temp_target}
                onChange={(e: InputEvent) => setConfig({ ...config, temp_target: e.target.value })}
              />
              <InputGroup
                label="Dung sai nhiệt độ (°C)"
                step="0.5"
                value={config.temp_tolerance}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, temp_tolerance: e.target.value })
                }
              />

              <div className="sm:col-span-2 mt-2">
                <InputGroup
                  label="Nhiệt độ kích hoạt phun sương tăng cường (°C)"
                  step="0.5"
                  value={config.misting_temp_threshold}
                  onChange={(e: InputEvent) =>
                    setConfig({ ...config, misting_temp_threshold: e.target.value })
                  }
                  desc="Nếu trời nóng vượt mức này, bơm sương sẽ chạy nhịp làm mát nhanh."
                />
              </div>

              <div className="sm:col-span-2 pt-4 pb-1">
                <span className="text-[9px] text-blue-400 font-black uppercase tracking-widest bg-blue-500/10 border border-blue-500/20 py-1.5 px-3 rounded-lg shadow-inner">
                  Nhịp phun cơ bản (Trời mát)
                </span>
              </div>
              <InputGroup
                label="Thời gian Phun (ms)"
                step="1000"
                value={config.misting_on_duration_ms}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, misting_on_duration_ms: e.target.value })
                }
                desc="1000ms = 1 Giây"
              />
              <InputGroup
                label="Thời gian Nghỉ (ms)"
                step="1000"
                value={config.misting_off_duration_ms}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, misting_off_duration_ms: e.target.value })
                }
              />

              <div className="sm:col-span-2 pt-4 pb-1 border-t border-slate-800">
                <span className="text-[9px] text-rose-400 font-black uppercase tracking-widest bg-rose-500/10 border border-rose-500/20 py-1.5 px-3 rounded-lg shadow-inner">
                  Nhịp làm mát nhanh (Trời nóng)
                </span>
              </div>
              <InputGroup
                label="Thời gian Phun (ms)"
                ...
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, high_temp_misting_on_duration_ms: e.target.value })
                }
              />
              <InputGroup
                label="Thời gian Nghỉ (ms)"
                ...
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, high_temp_misting_off_duration_ms: e.target.value })
                }
              />
            </div>
          </SubCard>
        ...
        <AccordionSection
          id="water"
          title="Quản Lý Bơm Nước"
          icon={Waves}
          color="text-cyan-400"
          isOpen={openSection === 'water'}
          onToggle={() => handleToggleSection('water')}
        >
          <SubCard title="Đo Mực Nước Bằng Siêu Âm">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                <InputGroup
                  label="Chiều cao từ cảm biến đến đáy bồn (cm)"
                  value={config.tank_height}
                  onChange={(e: InputEvent) =>
                    setConfig({ ...config, tank_height: e.target.value })
                  }
                />
              </div>
              <InputGroup
                label="Mức nước muốn giữ (%)"
                value={config.water_level_target}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, water_level_target: e.target.value })
                }
              />
              <InputGroup
                label="Dung sai bơm bù (%)"
                value={config.water_level_tolerance}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, water_level_tolerance: e.target.value })
                }
              />
              <InputGroup
                label="Mức báo động cạn (%)"
                value={config.water_level_min}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, water_level_min: e.target.value })
                }
              />
              <InputGroup
                label="Mức báo động tràn (%)"
                value={config.water_level_max}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, water_level_max: e.target.value })
                }
              />
              <div className="sm:col-span-2">
                <InputGroup
                  label="Mức nước khi xả cạn bồn (%)"
                  value={config.water_level_drain}
                  onChange={(e: InputEvent) =>
                    setConfig({ ...config, water_level_drain: e.target.value })
                  }
                />
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e: InputEvent) =>
                    setConfig({ ...config, circulation_mode: e.target.value })
                  }
                >
              ...
              {config.circulation_mode === 'timer' && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 animate-in fade-in slide-in-from-top-2">
                  <InputGroup
                    label="Bật sục khí (Giây)"
                    value={config.circulation_on_sec}
                    onChange={(e: InputEvent) =>
                      setConfig({ ...config, circulation_on_sec: e.target.value })
                    }
                  />
                  <InputGroup
                    label="Tắt sục khí (Giây)"
                    value={config.circulation_off_sec}
                    onChange={(e: InputEvent) =>
                      setConfig({ ...config, circulation_off_sec: e.target.value })
                    }
                  />
                </div>
              )}
            ...
            <div className="space-y-5">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-slate-300 tracking-wide uppercase">
                  Tự động bơm thêm nước ngầm
                </span>
                <Switch
                  isOn={config.auto_refill_enabled}
                  onClick={(val) => setConfig({ ...config, auto_refill_enabled: val })}
                  colorClass="bg-cyan-500 shadow-[0_0_10px_rgba(6,182,212,0.4)]"
                />
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-slate-300 tracking-wide uppercase">
                  Tự động xả nếu bồn đầy tràn
                </span>
                <Switch
                  isOn={config.auto_drain_overflow}
                  onClick={(val) => setConfig({ ...config, auto_drain_overflow: val })}
                  colorClass="bg-cyan-500 shadow-[0_0_10px_rgba(6,182,212,0.4)]"
                />
              </div>
              ...
              <div className="pt-4 border-t border-slate-800/50">
                <div className="flex items-center justify-between">
                  <div>
                    <span className="text-xs font-bold text-slate-300 tracking-wide uppercase">
                      Tự xả pha loãng khi quá liều EC
                    </span>
                    <p className="text-[10px] text-slate-500 mt-1">
                      Xả bớt dung dịch đậm đặc để tự bơm nước lã vào
                    </p>
                  </div>
                  <Switch
                    isOn={config.auto_dilute_enabled}
                    onClick={(val) => setConfig({ ...config, auto_dilute_enabled: val })}
                    colorClass="bg-cyan-500 shadow-[0_0_10px_rgba(6,182,212,0.4)]"
                  />
                </div>
                {config.auto_dilute_enabled && (
                  <div className="mt-4 pl-4 border-l-2 border-cyan-500/50 animate-in fade-in slide-in-from-left-2">
                    <InputGroup
                      label="Mức nước sẽ xả đi (cm)"
                      step="0.5"
                      value={config.dilute_drain_amount_cm}
                      onChange={(e: InputEvent) =>
                        setConfig({ ...config, dilute_drain_amount_cm: e.target.value })
                      }
                    />
                  </div>
                )}
          ...
          <SubCard title="Thay Nước Định Kỳ" className="mt-4">
            <div className="flex items-center justify-between mb-4">
              <span className="text-xs font-bold text-slate-300 tracking-wide uppercase">
                Bật lịch tự động xả nước cũ
              </span>
              <Switch
                isOn={config.scheduled_water_change_enabled}
                onClick={(val) => setConfig({ ...config, scheduled_water_change_enabled: val })}
                colorClass="bg-cyan-500 shadow-[0_0_10px_rgba(6,182,212,0.4)]"
              />
            </div>
            {config.scheduled_water_change_enabled && (
              <div className="grid grid-cols-1 gap-4 animate-in fade-in slide-in-from-top-2 bg-slate-900/50 p-4 rounded-xl border border-white/5 shadow-inner">
                ...
                <div className="space-y-3 border border-slate-700/50 p-3 rounded-lg bg-slate-950/50">
                  <InputGroup
                    type="text"
                    label="Giờ thay nước (Chuỗi Cron)"
                    value={config.water_change_cron}
                    onChange={(e: InputEvent) =>
                      setConfig({ ...config, water_change_cron: e.target.value })
                    }
                    desc="Cú pháp: Phút Giờ Ngày Tháng Thứ"
                  />

                  <div>
                    <span className="text-[10px] font-bold text-slate-500 uppercase flex items-center gap-1 mb-2">
                      <Clock size={10} /> Chọn nhanh lịch:
                    </span>
                    <div className="flex flex-wrap gap-2">
                      <button
                        onClick={() => setConfig({ ...config, water_change_cron: '0 0 7 * * SUN' })}
                        className="px-3 py-1.5 bg-slate-800 hover:bg-cyan-600 hover:text-white text-[11px] text-slate-300 rounded-md transition-colors border border-slate-700"
                      >
                        7h Sáng Chủ Nhật
                      </button>
                      <button
                        onClick={() =>
                          setConfig({ ...config, water_change_cron: '0 0 6 1,15 * *' })
                        }
                        className="px-3 py-1.5 bg-slate-800 hover:bg-cyan-600 hover:text-white text-[11px] text-slate-300 rounded-md transition-colors border border-slate-700"
                      >
                        Ngày 1 và 15 (6h Sáng)
                      </button>
                      <button
                        onClick={() => setConfig({ ...config, water_change_cron: '0 0 8 * * *' })}
                        className="px-3 py-1.5 bg-slate-800 hover:bg-cyan-600 hover:text-white text-[11px] text-slate-300 rounded-md transition-colors border border-slate-700"
                      >
                        8h Sáng mỗi ngày
                      </button>
                ...
                <InputGroup
                  label="Lượng xả đi mỗi lần (cm)"
                  value={config.scheduled_drain_amount_cm}
                  onChange={(e: InputEvent) =>
                    setConfig({ ...config, scheduled_drain_amount_cm: e.target.value })
                  }
                />
              </div>
            )}
        ...
        <AccordionSection
          id="dosing"
          title="Máy Pha Phân & Hóa Chất"
          icon={FlaskConical}
          color="text-fuchsia-400"
          isOpen={openSection === 'dosing'}
          onToggle={() => handleToggleSection('dosing')}
        >
          <SubCard title="Công Suất Bơm Vi Lượng (Chống giật tia)">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <InputGroup
                ...
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, dosing_pwm_percent: e.target.value })
                }
                desc="Giảm tốc độ để châm phân từ từ, chính xác hơn."
                errorText={dosingValidationErrors.dosing_pwm_percent}
              />
              <InputGroup
                label="Tốc độ Bơm Trộn (%)"
                step="1"
                value={config.osaka_mixing_pwm_percent}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, osaka_mixing_pwm_percent: e.target.value })
                }
              />
              <InputGroup
                label="Tốc độ Bơm Phun Sương (%)"
                step="1"
                value={config.osaka_misting_pwm_percent}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, osaka_misting_pwm_percent: e.target.value })
                }
              />
              <InputGroup
                label="Độ trễ khởi động bơm (ms)"
                step="100"
                value={config.soft_start_duration}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, soft_start_duration: e.target.value })
                }
                desc="Bảo vệ nguồn điện tử, tránh sụt áp đột ngột."
              />
            </div>
          </SubCard>
          ...
          <SubCard title="Châm Phân Bổ Sung Theo Giờ" className="mt-4">
            <div className="flex items-center justify-between mb-4">
              ...
              <Switch
                isOn={config.scheduled_dosing_enabled}
                onClick={(val) => setConfig({ ...config, scheduled_dosing_enabled: val })}
                colorClass="bg-fuchsia-500 shadow-[0_0_10px_rgba(217,70,239,0.4)]"
              />
            </div>
            {config.scheduled_dosing_enabled && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 animate-in fade-in slide-in-from-top-2 bg-slate-900/50 p-4 rounded-xl border border-white/5 shadow-inner">
                ...
                <div className="sm:col-span-2 space-y-3 border border-slate-700/50 p-3 rounded-lg bg-slate-950/50">
                  <InputGroup
                    type="text"
                    label="Lịch trình bơm (Chuỗi Cron)"
                    value={config.scheduled_dosing_cron}
                    onChange={(e: InputEvent) =>
                      setConfig({ ...config, scheduled_dosing_cron: e.target.value })
                    }
                    desc="Cú pháp: Phút Giờ Ngày Tháng Thứ"
                  />
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button
                        onClick={() =>
                          setConfig({ ...config, scheduled_dosing_cron: '0 0 6 * * *' })
                        }
                        className="px-3 py-1.5 bg-slate-800 hover:bg-fuchsia-600 hover:text-white text-[11px] text-slate-300 rounded-md transition-colors border border-slate-700"
                      >
                        6h Sáng mỗi ngày
                      </button>
                      <button
                        onClick={() =>
                          setConfig({ ...config, scheduled_dosing_cron: '0 0 8,16 * * *' })
                        }
                        className="px-3 py-1.5 bg-slate-800 hover:bg-fuchsia-600 hover:text-white text-[11px] text-slate-300 rounded-md transition-colors border border-slate-700"
                      >
                        8h Sáng & 4h Chiều
                      ...
                      <button
                        onClick={() =>
                          setConfig({ ...config, scheduled_dosing_cron: '0 0 7 * * SUN' })
                        }
                        className="px-3 py-1.5 bg-slate-800 hover:bg-fuchsia-600 hover:text-white text-[11px] text-slate-300 rounded-md transition-colors border border-slate-700"
                      >
                        7h Sáng Chủ Nhật
                ...
                <InputGroup
                  ...
                  onChange={(e: InputEvent) =>
                    setConfig({ ...config, scheduled_dose_a_ml: e.target.value })
                  }
                ...
                <InputGroup
                  ...
                  onChange={(e: InputEvent) =>
                    setConfig({ ...config, scheduled_dose_b_ml: e.target.value })
                  }
          ...
          <SubCard title="Cấu Hình Đảo Trộn Nước" className="mt-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <InputGroup
                label="Bao lâu đảo 1 lần (Giây)"
                step="60"
                value={config.scheduled_mixing_interval_sec}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, scheduled_mixing_interval_sec: e.target.value })
                }
              />
              <InputGroup
                label="Đảo trong bao lâu (Giây)"
                step="10"
                value={config.scheduled_mixing_duration_sec}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, scheduled_mixing_duration_sec: e.target.value })
                }
              />
              <div className="sm:col-span-2">
                <InputGroup
                  label="Đảo ngay sau khi châm phân (Giây)"
                  step="1"
                  value={config.active_mixing_sec}
                  onChange={(e: InputEvent) =>
                    setConfig({ ...config, active_mixing_sec: e.target.value })
                  }
                />
              </div>
              <div className="sm:col-span-2">
                <InputGroup
                  label="Thời gian chờ cảm biến ổn định số liệu (Giây)"
                  step="1"
                  value={config.sensor_stabilize_sec}
                  onChange={(e: InputEvent) =>
                    setConfig({ ...config, sensor_stabilize_sec: e.target.value })
                  }
                  desc="Tạm dừng đọc số sau khi trộn để tránh bị nhiễu do nước xáo trộn."
                />
              </div>
            </div>
          </SubCard>
          ...
          <SubCard title="Thuật Toán Châm & Lưu Lượng Thực Tế" className="mt-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <InputGroup
                label="Thể tích bồn chứa (Lít)"
                value={config.tank_volume_l}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, tank_volume_l: e.target.value })
                }
              />
              <InputGroup
                label="Thời gian chờ giữa Bơm A và B (Giây)"
                step="1"
                value={config.delay_between_a_and_b_sec}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, delay_between_a_and_b_sec: e.target.value })
                }
                desc="Chống kết tủa Canxi và Photpho"
              />

              <div className="sm:col-span-2 pt-4 pb-1 border-b border-slate-800">
                <span className="text-xs text-fuchsia-400 font-bold uppercase tracking-widest">
                  Đo lường đầu dò bơm
                </span>
              </div>
              <InputGroup
                label="Lưu lượng Bơm A (ml/giây)"
                step="0.1"
                min={0.1}
                value={config.pump_a_capacity_ml_per_sec}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, pump_a_capacity_ml_per_sec: e.target.value })
                }
                errorText={dosingValidationErrors.pump_a_capacity_ml_per_sec}
              />
              <InputGroup
                label="Lưu lượng Bơm B (ml/giây)"
                step="0.1"
                min={0.1}
                value={config.pump_b_capacity_ml_per_sec}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, pump_b_capacity_ml_per_sec: e.target.value })
                }
                errorText={dosingValidationErrors.pump_b_capacity_ml_per_sec}
              />
              <InputGroup
                label="Lưu lượng Bơm pH Tăng (ml/giây)"
                step="0.1"
                min={0.1}
                value={config.pump_ph_up_capacity_ml_per_sec}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, pump_ph_up_capacity_ml_per_sec: e.target.value })
                }
                errorText={dosingValidationErrors.pump_ph_up_capacity_ml_per_sec}
              />
              <InputGroup
                label="Lưu lượng Bơm pH Giảm (ml/giây)"
                step="0.1"
                min={0.1}
                value={config.pump_ph_down_capacity_ml_per_sec}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, pump_ph_down_capacity_ml_per_sec: e.target.value })
                }
                errorText={dosingValidationErrors.pump_ph_down_capacity_ml_per_sec}
              />
              ...
              <InputGroup
                label="Mức tăng EC khi châm 1ml"
                step="0.01"
                value={config.ec_gain_per_ml}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, ec_gain_per_ml: e.target.value })
                }
              />
              <InputGroup
                label="Hệ số rải phân EC (0-1)"
                step="0.1"
                value={config.ec_step_ratio}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, ec_step_ratio: e.target.value })
                }
                desc="Càng nhỏ máy sẽ châm càng từ từ, không bị quá tay."
              />
              <InputGroup
                label="Mức tăng pH khi châm 1ml"
                step="0.01"
                value={config.ph_shift_up_per_ml}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, ph_shift_up_per_ml: e.target.value })
                }
              />
              <InputGroup
                label="Mức giảm pH khi châm 1ml"
                step="0.01"
                value={config.ph_shift_down_per_ml}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, ph_shift_down_per_ml: e.target.value })
                }
              />
              <div className="sm:col-span-2">
                <InputGroup
                  label="Hệ số rải hóa chất pH (0-1)"
                  step="0.1"
                  value={config.ph_step_ratio}
                  onChange={(e: InputEvent) =>
                    setConfig({ ...config, ph_step_ratio: e.target.value })
                  }
                />
              </div>
            </div>
        ...
        <AccordionSection
          id="safety"
          title="Bảo Vệ Chống Chập/Hư Máy"
          icon={ShieldAlert}
          color="text-amber-400"
          isOpen={openSection === 'safety'}
          onToggle={() => handleToggleSection('safety')}
        >
          <SubCard title="Giới Hạn Báo Động (Còi Hú)" className="border-rose-500/20 bg-rose-500/5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <InputGroup
                label="Nhiệt độ báo động Lạnh (°C)"
                step="0.1"
                value={config.min_temp_limit}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, min_temp_limit: e.target.value })
                }
              />
              <InputGroup
                label="Nhiệt độ báo động Nóng (°C)"
                step="0.1"
                value={config.max_temp_limit}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, max_temp_limit: e.target.value })
                }
              />
              <InputGroup
                label="EC quá loãng (Báo động)"
                step="0.1"
                value={config.min_ec_limit}
                onChange={(e: InputEvent) => setConfig({ ...config, min_ec_limit: e.target.value })}
              />
              <InputGroup
                label="EC quá đặc (Báo động)"
                step="0.1"
                value={config.max_ec_limit}
                onChange={(e: InputEvent) => setConfig({ ...config, max_ec_limit: e.target.value })}
              />
              <InputGroup
                label="pH quá thấp (Báo động)"
                step="0.1"
                value={config.min_ph_limit}
                onChange={(e: InputEvent) => setConfig({ ...config, min_ph_limit: e.target.value })}
              />
              <InputGroup
                label="pH quá cao (Báo động)"
                step="0.1"
                value={config.max_ph_limit}
                onChange={(e: InputEvent) => setConfig({ ...config, max_ph_limit: e.target.value })}
              />
              <div className="sm:col-span-2">
                <InputGroup
                  label="Báo động hụt nước bồn (cm)"
                  value={config.water_level_critical_min}
                  onChange={(e: InputEvent) =>
                    setConfig({ ...config, water_level_critical_min: e.target.value })
                  }
                  desc="Tủ sẽ tự động ngắt điện mọi rơ-le để bảo vệ chống cháy máy bơm."
                />
              </div>
            </div>
          </SubCard>
          ...
          <SubCard title="Ngăn Bơm Chạy Quá Sức" className="mt-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <InputGroup
                label="Giới hạn ml bơm mỗi lần"
                value={config.max_dose_per_cycle}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, max_dose_per_cycle: e.target.value })
                }
              />
              <InputGroup
                label="Giới hạn ml bơm trong 1 giờ"
                value={config.max_dose_per_hour}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, max_dose_per_hour: e.target.value })
                }
              />
              <div className="sm:col-span-2">
                <InputGroup
                  label="Thời gian nghỉ để tản nhiệt bơm (Giây)"
                  value={config.cooldown_sec}
                  onChange={(e: InputEvent) =>
                    setConfig({ ...config, cooldown_sec: e.target.value })
                  }
                />
              </div>
              ...
              <div className="sm:col-span-2 pt-4 pb-1 border-t border-slate-800">
                <span className="text-xs text-amber-400 font-bold uppercase tracking-widest">
                  Bộ lọc sốc tín hiệu (Chống nhiễu)
                </span>
              </div>
              <InputGroup
                label="Bỏ qua nhiễu EC nếu nhảy đột ngột (Δ)"
                step="0.1"
                value={config.max_ec_delta}
                onChange={(e: InputEvent) => setConfig({ ...config, max_ec_delta: e.target.value })}
              />
              <InputGroup
                label="Bỏ qua nhiễu pH nếu nhảy đột ngột (Δ)"
                step="0.1"
                value={config.max_ph_delta}
                onChange={(e: InputEvent) => setConfig({ ...config, max_ph_delta: e.target.value })}
              />
              <InputGroup
                label="Chênh lệch EC tối thiểu để bắt đầu châm"
                step="0.01"
                value={config.ec_ack_threshold}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, ec_ack_threshold: e.target.value })
                }
              />
              <InputGroup
                label="Chênh lệch pH tối thiểu để bắt đầu châm"
                step="0.01"
                value={config.ph_ack_threshold}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, ph_ack_threshold: e.target.value })
                }
              />
              <div className="sm:col-span-2">
                <InputGroup
                  label="Chênh lệch Nước (%) tối thiểu để kích hoạt bơm"
                  step="0.1"
                  value={config.water_ack_threshold}
                  onChange={(e: InputEvent) =>
                    setConfig({ ...config, water_ack_threshold: e.target.value })
                  }
                />
              </div>
            </div>
          ...
          <SubCard title="Bảo Vệ Máy Bơm Nước Chống Cháy" className="mt-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <InputGroup
                label="Số lần bơm nước lã tối đa / Giờ"
                value={config.max_refill_cycles_per_hour}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, max_refill_cycles_per_hour: e.target.value })
                }
              />
              <InputGroup
                label="Thời gian chạy bơm rốn tối đa (Giây)"
                value={config.max_refill_duration_sec}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, max_refill_duration_sec: e.target.value })
                }
                desc="Ngắt máy bơm nước lên nếu quá thời gian (chống kẹt hụt nước ngầm)"
              />
              <InputGroup
                label="Số lần xả cặn tối đa / Giờ"
                value={config.max_drain_cycles_per_hour}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, max_drain_cycles_per_hour: e.target.value })
                }
              />
              <InputGroup
                label="Thời gian chạy van xả tối đa (Giây)"
                value={config.max_drain_duration_sec}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, max_drain_duration_sec: e.target.value })
                }
              />
            </div>
          </SubCard>
        ...
        <AccordionSection
          id="sensor"
          title="Cảm Biến & Cân Chỉnh (Calib)"
          icon={Activity}
          color="text-indigo-400"
          isOpen={openSection === 'sensor'}
          onToggle={() => handleToggleSection('sensor')}
        >
            ...
            <div className="space-y-5">
              <div>
                ...
                <select
                  ...
                  onChange={(e: InputEvent) =>
                    setConfig({ ...config, publish_interval: parseInt(e.target.value) })
                  }
                >
          ...
          <SubCard
            title="Đọc Tín Hiệu Các Đầu Dò"
            className="mb-4 bg-indigo-900/20 border-indigo-500/20"
          >
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Switch
                  isOn={config.is_ph_enabled}
                  onClick={(val) => setConfig({ ...config, is_ph_enabled: val })}
                  colorClass="bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.5)]"
                />
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-widest text-slate-300">
                  Nhận tín hiệu Dinh dưỡng (EC)
                </span>
                <Switch
                  isOn={config.is_ec_enabled}
                  onClick={(val) => setConfig({ ...config, is_ec_enabled: val })}
                  colorClass="bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.5)]"
                />
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-widest text-slate-300">
                  Nhận tín hiệu Nhiệt độ nước
                </span>
                <Switch
                  isOn={config.is_temp_enabled}
                  onClick={(val) => setConfig({ ...config, is_temp_enabled: val })}
                  colorClass="bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.5)]"
                />
              </div>
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-widest text-slate-300">
                  Nhận tín hiệu Radar Mực nước
                </span>
                <Switch
                  isOn={config.is_water_level_enabled}
                  onClick={(val) => setConfig({ ...config, is_water_level_enabled: val })}
                  colorClass="bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.5)]"
                />
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="px-5 py-4 bg-indigo-500/10 border border-indigo-500/30 rounded-2xl flex items-start space-x-3 shadow-inner">
                ...
                <p className="text-xs text-indigo-200 leading-relaxed font-medium">
                  Chuẩn bị dung dịch chuẩn sạch, rửa đầu dò bằng nước cất trước mỗi lần đo. Wizard
                  sẽ tự gọi API <b>capture</b> ở từng điểm, không cần nhập điện áp thủ công.
                </p>
              </div>
              ...
              <div className="rounded-xl border border-cyan-500/20 bg-cyan-500/5 p-4 space-y-3">
                <p className="text-xs font-black uppercase tracking-widest text-cyan-300">
                  Pha triển khai hệ số (theo device_id)
                </p>
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-slate-200">
                      Pha 1 - Observe (thu mẫu + tính hệ số đề xuất)
                    </span>
                    <Switch
                      isOn={adaptivePhases.observe}
                      onClick={(val) => saveAdaptivePhases({ ...adaptivePhases, observe: val })}
                      colorClass="bg-cyan-500"
                    />
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-slate-200">
                      Pha 2 - Recommend (so sánh hệ số cũ/mới + xác nhận tay)
                    </span>
                    <Switch
                      isOn={adaptivePhases.recommend}
                      onClick={(val) => saveAdaptivePhases({ ...adaptivePhases, recommend: val })}
                      colorClass="bg-cyan-500"
                    />
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-slate-200">
                      Pha 3 - Auto Apply (tự áp dụng nếu đủ điều kiện)
                    </span>
                    <Switch
                      isOn={adaptivePhases.auto_apply}
                      onClick={(val) => saveAdaptivePhases({ ...adaptivePhases, auto_apply: val })}
                      colorClass="bg-cyan-500"
                    />
                  </div>
                </div>
              ...
              {isCalibrationBlocked && (
                <div className="p-4 rounded-xl border border-rose-500/30 bg-rose-500/10 text-rose-100">
                  <p className="text-xs font-black uppercase tracking-wider mb-2">
                    Không thể bắt đầu calib
                  </p>
                  <ul className="text-xs space-y-1 list-disc pl-4">
                    {!isSensorOnline && (
                      <li>Sensor đang offline. Kiểm tra nguồn cảm biến hoặc kết nối mạng.</li>
                    )}
                    {isPhError && (
                      <li>
                        err_ph=true. Rửa đầu dò, kiểm tra dây tín hiệu và chờ nhiệt độ ổn định trước
                        khi đo lại.
                      </li>
                    )}
                    <li>
                      Đảm bảo đầu dò ngập đủ dung dịch chuẩn và không có bọt khí bám đầu cảm biến.
                    </li>
                  </ul>
                </div>
              )}
              ...
              {wizardStep < calibrationPoints.length ? (
                <div className="p-4 rounded-xl border border-indigo-500/25 bg-slate-900/60 space-y-3">
                  ...
                  <p className="text-sm font-bold text-slate-100">
                    Nhúng đầu dò vào dung dịch{' '}
                    <span className="text-indigo-300">pH {activePoint}</span>, sau đó bấm{' '}
                    <span className="text-emerald-300">Bắt đầu đo</span>.
                  </p>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {isCapturingPoint && (
                      <span className="text-xs text-amber-300 font-bold flex items-center gap-1">
                        ...
                        {stabilityStatus === 'stable' ? 'Đã ổn định' : 'Đang chờ ổn định'}
                      </span>
                    )}
                    ...
                    {capturedPoints[activePoint] && !isCapturingPoint && (
                      <>
                        <span className="text-xs text-emerald-300 font-bold">
                          Đã ghi nhận: {capturedPoints[activePoint].voltage.toFixed(3)}V
                        </span>
                        <button
                ...
                <div className="p-4 rounded-xl border border-emerald-500/30 bg-emerald-500/10 space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
                    <div className="rounded-lg bg-slate-900/60 p-3 border border-white/10">
                      ...
                      <p className="font-black text-slate-100">
                        {calibrationSummary.ph_v7 ?? '--'} V
                      </p>
                    </div>
                    <div className="rounded-lg bg-slate-900/60 p-3 border border-white/10">
                      <p className="text-slate-400 text-xs">ph_v4</p>
                      <p className="font-black text-slate-100">
                        {calibrationSummary.ph_v4 ?? '--'} V
                      </p>
                    </div>
                    <div className="rounded-lg bg-slate-900/60 p-3 border border-white/10">
                  ...
                  {calibrationSummary.ph_v10 !== null && (
                    <p className="text-xs text-slate-300">
                      Điểm mở rộng pH 10: <b>{calibrationSummary.ph_v10}V</b> (dùng để đánh giá
                      tuyến tính).
                    </p>
                  )}
                  <div className="rounded-lg bg-slate-900/50 border border-white/10 p-3 text-xs text-slate-200 space-y-2">
                    ...
                    <p>
                      Δ ph_v7: <b>{calibrationDeviation.ph_v7 ?? '--'} V</b> · Δ ph_v4:{' '}
                      <b>{calibrationDeviation.ph_v4 ?? '--'} V</b>
                    </p>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {adaptivePhases.auto_apply && (
                      <button
                        onClick={async () => {
                          const confidenceOk =
                            calibrationSummary.reliability >= adaptivePhases.confidence_threshold;
                          if (!confidenceOk) {
                            toast.error(
                              `Confidence ${calibrationSummary.reliability}% chưa đạt ngưỡng ${adaptivePhases.confidence_threshold}%`,
                            );
                            return;
                          }
                          if (hasSafetyWarningIn24h) {
                            toast.error(
                              'Trong 24h gần nhất có cảnh báo an toàn. Auto-apply bị chặn.',
                            );
                            return;
                          }
                          const nextConfig = applyCalibrationToConfig();
          ...
          <SubCard title="Thông Số Calib Analog (khác)">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <InputGroup
                label="Hệ số nhân EC (K Factor)"
                step="1.0"
                value={config.ec_factor}
                onChange={(e: InputEvent) => setConfig({ ...config, ec_factor: e.target.value })}
              />
              <InputGroup
                label="Bù trừ sai số EC tĩnh (Offset)"
                step="0.1"
                value={config.ec_offset}
                onChange={(e: InputEvent) => setConfig({ ...config, ec_offset: e.target.value })}
              />
              <InputGroup
                label="Bù sai số Nhiệt độ đo được (Offset)"
                step="0.1"
                value={config.temp_offset}
                onChange={(e: InputEvent) => setConfig({ ...config, temp_offset: e.target.value })}
              />
              <InputGroup
                label="Hệ số bù Nhiệt cho EC (Beta)"
                step="0.01"
                value={config.temp_compensation_beta}
                onChange={(e: InputEvent) =>
                  setConfig({ ...config, temp_compensation_beta: e.target.value })
                }
              />
            </div>
          </SubCard>
