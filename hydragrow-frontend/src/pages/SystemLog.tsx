...
const DosingMetadata = ({ meta }: { meta: any }) => {
  ...
  if (cycleMeta.stabilized_window_sec != null) targetRows.push({ label: 'Độ trễ tĩnh bồn tự học:', value: `${cycleMeta.stabilized_window_sec} giây`, accent: 'text-indigo-700 font-bold' });
  if (cycleMeta.kalman?.matrix_update_count != null) targetRows.push({ label: 'Lượt cập nhật ma trận:', value: `${cycleMeta.kalman.matrix_update_count}`, accent: 'text-emerald-700 font-bold' });
...
const WaterMetadata = ({ meta }: { meta: any }) => {
  ...
  if (meta.level_before != null && meta.level_after != null) {
    ...
    rows.push({ label: 'Hành trình mức nước bồn:', value: `${meta.level_before.toFixed(1)}cm → ${meta.level_after.toFixed(1)}cm (${delta >= 0 ? '+' : ''}${delta.toFixed(1)})`, accent: 'text-blue-700 font-bold' });
  }
  ...
  if (meta.success != null) rows.push({ label: 'Kết quả chu kỳ thủy lực:', value: meta.success ? 'Thành công' : 'Lỗi Timeout cấp nước', accent: meta.success ? 'text-emerald-700 font-bold' : 'text-red-700 font-bold' });
  ...
  return (
    <div className="mt-3 flex flex-col gap-1.5 text-xs font-medium bg-emerald-50/80 border border-emerald-100 rounded-xl px-3 py-2.5">
      <div className="text-[9px] font-black text-emerald-700/75 mb-0.5 uppercase tracking-wider">Thông số chu trình thủy lực</div>
      {rows.map(r => (
        <div key={r.label} className="flex items-center justify-between border-b border-white/5 last:border-transparent pb-1 last:pb-0">
          <span className="text-emerald-800/80 text-[11px]">{r.label}</span>
          <span className={`${r.accent ?? 'text-emerald-900'} text-[11px]`}>{r.value}</span>
        </div>
      ))}
    </div>
...
const AlertMetadata = ({ meta }: { meta: any }) => {
  if (!meta) return null;
  const rows: { label: string; value: string; accent?: string }[] = [];
  if (meta.alert_type) rows.push({ label: 'Mã thuật toán chẩn đoán:', value: String(meta.alert_type), accent: 'text-red-700 font-bold font-mono' });
  if (meta.source) rows.push({ label: 'Khối phần cứng phát hiện:', value: String(meta.source), accent: 'text-orange-400 font-medium' });
  if (meta.retry_count != null) rows.push({ label: 'Vòng lặp thử lại châm bù:', value: `${meta.retry_count} lần`, accent: 'text-amber-800 font-mono' });
  if (meta.limit_value != null) rows.push({ label: 'Ngưỡng kịch trần ranh giới:', value: String(meta.limit_value), accent: 'text-red-700 font-mono' });
  ...
  return (
    <div className="mt-3 flex flex-col gap-1.5 text-xs font-medium bg-rose-950/10 border border-rose-900/20 rounded-xl px-3 py-2.5">
      <div className="text-[9px] font-black text-red-700/70 mb-0.5 uppercase tracking-wider flex items-center gap-1"><ShieldAlert size={10} /> Root Cause Diagnostic Snapshot</div>
      {rows.map(r => (
        <div key={r.label} className="flex items-center justify-between border-b border-white/5 last:border-transparent pb-1 last:pb-0">
          <span className="text-emerald-800/80 text-[11px]">{r.label}</span>
          <span className={`${r.accent ?? 'text-emerald-900'} text-[11px]`}>{r.value}</span>
...
const CalibrationMetadata = ({ meta }: { meta: any }) => {
  if (!meta) return null;
  const rows: { label: string; value: string; accent?: string }[] = [];
  if (meta.parameter) rows.push({ label: 'Trục ma trận thích ứng:', value: String(meta.parameter), accent: 'text-purple-700 font-bold' });
  if (meta.old_value != null && meta.new_value != null) {
    rows.push({ label: 'Ma trận tiến hóa (Kalman):', value: `${Number(meta.old_value).toFixed(4)} → ${Number(meta.new_value).toFixed(4)}`, accent: 'text-emerald-700 font-mono font-bold' });
  }
  if (meta.skip_reason) rows.push({ label: 'Lý do bộ lọc chặn cập nhật:', value: String(meta.skip_reason), accent: 'text-amber-800 font-medium' });

  if (rows.length === 0) return null;
  return (
    <div className="mt-3 flex flex-col gap-1.5 text-xs font-medium bg-emerald-50/80 border border-emerald-100 rounded-xl px-3 py-2.5">
      <div className="text-[9px] font-black text-emerald-700/75 mb-0.5 uppercase tracking-wider">MIMO Gain Matrix Evolution</div>
      {rows.map(r => (
        <div key={r.label} className="flex items-center justify-between border-b border-white/5 last:border-transparent pb-1 last:pb-0">
          <span className="text-emerald-800/80 text-[11px]">{r.label}</span>
          <span className={`${r.accent ?? 'text-emerald-900'} text-[11px]`}>{r.value}</span>
...
const FsmBadge = ({ message }: { message: string }) => {
  const stateMap: Record<string, { label: string; color: string }> = {
    ...
    'MimoDosing': { label: 'Đang châm MIMO', color: 'text-cyan-700 bg-cyan-500/10 border-cyan-500/20' },
    'ActiveMixing': { label: 'Sục khuấy động', color: 'text-purple-700 bg-purple-500/10 border-purple-500/20' },
    'Stabilizing': { label: 'Lắng bão hòa', color: 'text-amber-800 bg-amber-500/10 border-amber-500/20' },
    'Cooldown': { label: 'Nghỉ dưỡng bồn', color: 'text-amber-800 bg-amber-500/10 border-amber-500/20' },
    'Monitoring': { label: 'Chăm sóc tự động', color: 'text-emerald-800/80 bg-emerald-100 border-emerald-200' },
    'Idle': { label: 'Hệ thống nghỉ', color: 'text-emerald-800/80 bg-emerald-100 border-emerald-200' },
        ...
        {hasMetadata && (
          <div className="mt-2.5 pt-2 border-t border-emerald-100 flex flex-col items-start">
            <button
              ...
              className="flex items-center gap-1 text-[10px] font-bold text-emerald-700/75 hover:text-emerald-800/80 tracking-wide uppercase transition-colors"
            >
    ...
    <div className="p-4 md:p-8 max-w-3xl mx-auto pb-28 text-emerald-950">
      ...
      <div className="bg-white/90 border border-emerald-100 rounded-3xl p-4 mb-8 flex flex-col md:flex-row justify-between items-stretch md:items-center gap-4 relative z-10 backdrop-blur-md">
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold transition-all duration-200 border whitespace-nowrap
                  ${active
                    ...
                    : 'bg-white text-emerald-800/80 border-emerald-100 hover:bg-white hover:text-emerald-950'
                  }`}
              >
                <Icon size={12} />
                {btn.label}
        ...
        <button
          ...
          className="flex items-center justify-center space-x-2 bg-emerald-100 hover:bg-emerald-200 disabled:opacity-40 text-white px-4 py-1.5 rounded-xl border border-emerald-200 transition-all duration-200 shadow-sm text-xs font-bold shrink-0 active:scale-95"
        >
          <Download size={13} className={systemEvents.length > 0 ? "text-emerald-700" : "text-emerald-700/75"} />
          <span>Xuất tệp</span>
        </button>
      </div>
      ...
      {isLoading ? (
        <div className="flex items-center justify-center gap-2.5 py-24 text-emerald-700/75">
          ...
          <span className="text-xs font-semibold uppercase tracking-wider text-emerald-800/80">Đang đồng bộ dòng thời gian...</span>
        </div>
