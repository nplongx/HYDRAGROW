 ...
 * Mock auth chỉ dùng local development và phải có backend-issued dev token.
 */
function withAuthHeader(init?: RequestInit): RequestInit | undefined {
  ...
  if (
    ...
    !headers.has('X-Dev-Auth-Token')
  ) {
    const devAuthToken = import.meta.env.VITE_DEV_AUTH_TOKEN?.trim();
    if (devAuthToken) headers.set('X-Dev-Auth-Token', devAuthToken);
  }
