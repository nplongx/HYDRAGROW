...
impl Tank {
    ...
    pub fn step(
        ...
        if actuators.pump_a.on {
            let pwm_factor = actuators.pump_a.pwm as f32 / 100.0;
            let flow = config.pump_a_capacity_ml_per_sec * dt_sec * pwm_factor;
            ec_change += (flow * config.ec_gain_per_ml) / current_vol;
        }
        if actuators.pump_b.on {
            let pwm_factor = actuators.pump_b.pwm as f32 / 100.0;
            let flow = config.pump_b_capacity_ml_per_sec * dt_sec * pwm_factor;
            ec_change += (flow * config.ec_gain_per_ml) / current_vol;
        }
        if actuators.pump_ph_up.on {
            let pwm_factor = actuators.pump_ph_up.pwm as f32 / 100.0;
            let flow = config.pump_ph_up_capacity_ml_per_sec * dt_sec * pwm_factor;
            ph_change += (flow * config.ph_shift_up_per_ml) / current_vol;
        }
        if actuators.pump_ph_down.on {
            let pwm_factor = actuators.pump_ph_down.pwm as f32 / 100.0;
            let flow = config.pump_ph_down_capacity_ml_per_sec * dt_sec * pwm_factor;
            ph_change -= (flow * config.ph_shift_down_per_ml) / current_vol;
        }
